...
bool PhysBody::Contains(int x, int y) const
{
	...
	// is inside ANY of the shapes contained by this body ----- Testpoint function
	
	for (b2Fixture* it = body->GetFixtureList() ; it != nullptr ; it = it->GetNext() )
	{

		if (it->GetShape()->TestPoint(body->GetTransform(), b2Vec2( PIXEL_TO_METERS (x), PIXEL_TO_METERS (y))))
		{
			return true;
		}
	}
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	for (b2Fixture* it = body->GetFixtureList(); it != nullptr; it = it->GetNext())
	{
		...
		if (it->GetShape()->RayCast(&output, input, transform, childIndex))
		{
			normal_x = output.normal.x;
			normal_y = output.normal.y;
			b2Vec2 temp = input.p2 - input.p1;

			ret = METERS_TO_PIXELS((output.fraction)* temp.Length());
		}
